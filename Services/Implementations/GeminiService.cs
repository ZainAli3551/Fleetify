    ...
    public class GeminiService : IGeminiService
    {
        ...
        public async Task<string?> GenerateReplyAsync(string userMessage, List<SupportMessage>? conversationHistory = null, string? liveContext = null)
        {
            ...
            try
            {
                ...
                systemInstructionText.AppendLine("  - Rate: $1.50 per km per kg");
                systemInstructionText.AppendLine("  - Exact Formula: Total Cost = Distance (in km) × Weight (in kg) × $1.50");
                systemInstructionText.AppendLine("  - IMPORTANT: There is NO separate base rate or separate distance/weight charges. They are multiplied together.");
                systemInstructionText.AppendLine("  - Example 1: Distance = 90.6 km, Weight = 2 kg -> Calculation: 90.6 km × 2 kg × $1.50 = $271.80.");
                systemInstructionText.AppendLine("  - Example 2: Distance = 80 km, Weight = 20 kg -> Calculation: 80 km × 20 kg × $1.50 = $2,400.00.");
                systemInstructionText.AppendLine("  - COST CALCULATION RULE: Whenever a customer asks for price calculation, or mentions distance and weight, ALWAYS multiply (Distance in km × Weight in kg × $1.50) step-by-step and state the final calculated price clearly!");
