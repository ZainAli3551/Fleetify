    ...
    public class GeminiService : IGeminiService
    {
        ...
        public async Task<string?> GenerateReplyAsync(string userMessage, List<SupportMessage>? conversationHistory = null, string? liveContext = null)
        {
            ...
            try
            {
                ...
                systemInstructionText.AppendLine("  - Base Formula: Base Cost = 1.5 × Distance (km) × Weight (kg)");
                systemInstructionText.AppendLine("  - Distance-Based Petrol Surcharges:");
                ...
                systemInstructionText.AppendLine("  - Delivery Types & Speed Multipliers:");
                systemInstructionText.AppendLine("    • Normal Delivery: Multiplier 1.0x (Standard 1-2 business days)");
                systemInstructionText.AppendLine("    • Fast Delivery: Multiplier 1.75x (Next-day priority delivery)");
                systemInstructionText.AppendLine("    • Express Delivery: Multiplier 2.5x (Same-day rush delivery)");
                systemInstructionText.AppendLine("  - Final Formula: Total Cost = ((1.5 × Distance_km × Weight_kg) + Petrol_Surcharge) × Delivery_Multiplier");
                systemInstructionText.AppendLine("  - Example 1: Distance = 4.4 km, Weight = 5 kg, Normal -> Base = 1.5 × 4.4 × 5 = 33, Petrol = 200. Total = (33 + 200) × 1.0 = Rs. 233.00.");
                systemInstructionText.AppendLine("  - Example 2: Distance = 15 km, Weight = 2 kg, Fast -> Base = 1.5 × 15 × 2 = 45, Petrol = 500. Total = (45 + 500) × 1.75 = Rs. 953.75.");
                systemInstructionText.AppendLine("  - COST CALCULATION RULE: Whenever a customer asks for price calculation or quotes, compute step-by-step using this exact formula with petrol surcharge and delivery multiplier, and state the price in Rs. clearly!");
